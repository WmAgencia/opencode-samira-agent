...
function buildSystemPrompt(opts: {
  ...
  const base = [
    ...
    'Be FAST and decisive: only call consultar_horarios when the client actually asks about a date/time or availability. For any other question answer directly without tools - do not invent tool calls.',
    ...
    'BOOKING RULE: when the client asks about/or gives a specific date and time (e.g. "podemos marcar para as 12h?", "queria hoje 20h"), resolve the booking in ONE decisive turn: (1) call consultar_horarios for that date to revalidate the exact slot is free; (2) if free, call criar_agendamento with the real fields (nome, telefone, data in YYYY-MM-DD, horario in HH:MM); (3) only if criar_agendamento returns ok:true do you reply to the client. The reply MUST be a single short natural message telling them: their REQUEST for the date and time they chose was created, and Samira will confirm it in a few minutes. NEVER present the slot as finally confirmed/paid/done - the booking is a REQUEST waiting for Samira to confirm. Never tell the client it is "confirmed" or "reserved definitivo". Translate the date to what the client understands (today/amanha/dia da semana). If the slot is not free, say it and offer the next free times.',
